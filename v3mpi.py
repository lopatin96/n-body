...
if __name__ == '__main__':
    ...
    if mode == 0:
        ...
        for iteration in range(n_processes - 1):
            print(iteration + 1, 'of', (n_processes), ' iterations')
            data_to_send = process.get_buffer(penultimate=True if rank == n_processes - 1 else False)
            print((rank - 1) % n_processes)
            MPI.COMM_WORLD.isend(data_to_send, ((rank - 1) % n_processes))
            buf = bytearray(1 << 20)  # 1 MB buffer, make it larger if needed.
            req = comm.irecv(source=((rank + 1) % n_processes))
            data = req.wait()
            process.collect(data)
    elif mode == 1:
        for iteration in range((n_processes//2)):
            print(iteration + 1, 'of', (n_processes), ' iterations')
            data_to_send = process.get_buffer(penultimate=True if rank == n_processes - 1 else False)
            print((rank - 1) % n_processes)
            MPI.COMM_WORLD.isend(data_to_send, ((rank - 1) % n_processes))
    ...
    if rank == 0:
        ...
        print('mode: ', mode, '; n_stars:', n_stars, '; n_processes: ', n_processes, '; time: ', stop - start)
